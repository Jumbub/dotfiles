...
f.clockWidget = function() return wibox.widget.textclock("%d/%m %H:%M") end
...
f.setWallpaper = function(s)
    ...
    gears.wallpaper.maximized("/home/jamie/.config/awesome/wallpaper1.png", s,
                              false)
end
...
f.setupGlobalBindings = function()
    ...
    root.keys(gears.table.join(awful.key({modkey}, "j", function()
        awful.client.focus.byidx(1)
    end), awful.key({modkey}, "k", function() awful.client.focus.byidx(-1) end),
                               awful.key({modkey}, "Down", function()
        awful.client.incwfact(0.1)
    end), awful.key({modkey}, "Up", function() awful.client.incwfact(-0.1) end),
                               awful.key({modkey}, "l", function()
        awful.tag.incmwfact(0.05)
    end), awful.key({modkey}, "h", function() awful.tag.incmwfact(-0.05) end),
                               awful.key({modkey, "Shift"}, "j", function()
        awful.client.swap.byidx(1)
    end), awful.key({modkey, "Shift"}, "k",
                    function() awful.client.swap.byidx(-1) end),
                               awful.key({modkey, "Control"}, "j", function()
        awful.screen.focus_relative(1)
    end), awful.key({modkey, "Control"}, "k",
                    function() awful.screen.focus_relative(-1) end),
                               awful.key({modkey, "Shift"}, "h", function()
        awful.tag.incnmaster(1, nil, true)
    end), awful.key({modkey, "Shift"}, "l",
    ...
    end), awful.key({modkey}, "Return", function() awful.spawn(terminal) end),
                               awful.key({modkey}, "e",
                                         function() awful.spawn(guieditor) end),
                               awful.key({modkey}, "r", function()
        awful.spawn("/usr/bin/rofi -show run")
    end), awful.key({modkey}, "c", function()
        awful.spawn("/home/jamie/scripts/clipboard-manager")
    end), awful.key({modkey, "Control"}, "r", awesome.restart), awful.key(
                                   {modkey, "Control"}, "n", unminimiseClient)))
end
...
f.clientKeyBindings = function()
    return gears.table.join(awful.key({modkey}, "f", function(c)
        c.fullscreen = not c.fullscreen
        c:raise()
    end), awful.key({modkey}, "x", function(c) c:kill() end), awful.key(
                                {modkey}, "o",
                                function(c) c:move_to_screen() end), awful.key(
                                {modkey}, "n",
                                function(c) c.minimized = true end), awful.key(
                                {modkey}, "m",
                                function() awful.layout.inc(1) end),
                            awful.key({modkey, "Shift"}, "m", function(c)
        c.maximized = not c.maximized
        c:raise()
    end), awful.key({modkey, "Control"}, "h", function(c)
        awful.client.setmaster(c)
        awful.tag.incnmaster(1, nil, true)
    end), awful.key({modkey, "Control"}, "l", function(c)
    ...
    awful.rules.rules = {
        {
            ...
            rule_any = {
                ...
                class = {
                    ...
                    "MessageWin", "Sxiv", "Tor Browser", "Wpa_gui", "veromix",
                    "xtightvncviewer"
                },
                name = {"Event Tester"},
...
f.setupScreens = function(screen)
    ...
    local cpuWrapped = lain.widget.cpu {
        ...
        settings = function()
            widget:clear()
            for i, v in ipairs(cpu_now) do widget:add_value(v.usage) end
        end
    }
    ...
    local batWrapped = lain.widget.bat {
        widget = bat,
        settings = function()
            widget:clear()
    ...
    local volume = lain.widget.alsabar {
        ...
        paddings = 0,
        margins = 1,
        colors = {background = "#222", mute = "#baa", unmute = "#666"},
    ...
    volume.bar:buttons(awful.util.table.join(
                           awful.button({}, 1, function()
            -- right click
            os.execute(string.format("%s set %s toggle", volume.cmd,
                                     volume.togglechannel or volume.channel))
            volume.update()
        end), awful.button({}, 4, function()
            -- scroll up
            os.execute(string.format("%s set %s 1%%+", volume.cmd,
                                     volume.channel))
            volume.update()
